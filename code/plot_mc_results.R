...
list(baseline = baseline_metric, robust = robust_metric) %>%
  lapply(summary_stat) %>%
  bind_rows(.id = 'model') %>%
  mutate(year = year_vec[t]) %>%
  mutate(model = factor(model, c('baseline', 'robust'), c('Baseline', 'Robust'))) %>%
  ggplot(aes(x=year)) +
...
plot_shaded_diff <- function(recourse = NULL, loop_vec = 1:10, default = c(1,4,0.25,1,5), dep_var = 1, interval = c(0.05, 0.95), realisation = NULL) {
  base_string <- "run_sp-%s_tt-%s_kt-%s_ns-%s_r-%s"
  ...
  summary_diff_pct %>%
    ...
    geom_ribbon(aes(ymin = lb, ymax = ub, fill = recourse), alpha = 0.3) +
...
plot_shaded_diff(loop_vec = c(2,3,4,5,6,7), dep_var = 2, realisation = 1:5)
plot_shaded_diff(loop_vec = c(0.1, 0.15, 0.2, 0.25), dep_var = 3, realisation = 1:5)
plot_shaded_diff(loop_vec = 1:12, dep_var = 4, realisation = 1:100, interval = c(0.25, 0.75))
