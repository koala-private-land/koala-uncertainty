...
function fcn_two_stage_opt(C::Matrix, M::Array{Float64,3}, K::Float64, p::Vector=(ones(size(M,3))/size(M,3)), tt::Integer = Integer(round(size(M,2)/2)); β::Vector=zeros(size(M,1)), γ::Vector=zeros(size(M,1)), add_recourse = true, terminate_recourse = true)
